...
export default function CreatorScreen() {
    ...
    async function loadUserData() {
      try {
        ...
        if (isMounted) {
          ...
          await loadCreatorData(parsedData.uId);
          await loadPublicationsData(parsedData.uId);
        } else {
  ...
  // Render publications
  const renderPublicationItem = ({ item }: { item: publicationItemsProps }) => {
      ...
      <View style={styles.centralPart}>
        ...
        <View style={styles.publicationsWrapper}>
          ...
          {publications.length > 0 ? (
            ...
            <Text style={styles.emptyText}>Опубликуйте первое творение!</Text>
          )}
        </View>
      </View>
      <Footer />
